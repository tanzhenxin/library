...
public class WeiboSsoHandler {
	...
	private OAuth2Listener mAuthDialogListener;
	private OAuth2AccessToken mAccessToken;
	private Activity mAuthActivity;

	public WeiboSsoHandler(Activity activity, OAuth2Provider authProvider) {
	 ...
	 *            用于接收认证信息的监听者
	 */
	public void authorize(final OAuth2Listener listener) {
		authorize(DEFAULT_AUTH_ACTIVITY_CODE, listener);
	}

	private void authorize(int activityCode, final OAuth2Listener listener) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				if (error != null) {
					if (error.equals("access_denied")
						...
						mAuthDialogListener.onError(new OAuth2Exception(error,
					...
					mAccessToken.setToken(data.getStringExtra(OAuth2AccessToken.KEY_TOKEN));
					mAccessToken.setExpiresIn(data
							.getStringExtra(OAuth2AccessToken.KEY_EXPIRES));
					mAccessToken.setRefreshToken(data
							.getStringExtra(OAuth2AccessToken.KEY_REFRESHTOKEN));
				...
				if (data != null) {
					...
					mAuthDialogListener.onError(new OAuth2Exception(data
